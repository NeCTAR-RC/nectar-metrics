...
def test_az_metrics():
    for metric in naming.AZ_METRICS:
        assert naming.from_dotted_path('az.melbourne-qh2.%s' % metric) == \
            ('nectar_%s' % metric, {'az': 'melbourne-qh2'})
...
def test_users_total():
    assert naming.from_dotted_path('users.total') == \
        ('nectar_users_total', {})


def test_active_projects():
    assert naming.from_dotted_path('active.projects.compute') == \
        ('nectar_active_projects', {'service': 'compute'})
