...
class AllocationStatusPollster(AllocationPollsterBase):
    def get_samples(self, manager, cache, resources):
        ...
        for status, count in counts.items():
            samples.append(
                sample.Sample(
                    name=f'global.allocations.{status}',
                    type=sample.TYPE_GAUGE,
                    unit='Allocation',
                    volume=count,
                    user_id=None,
        ...
        for allocation in resources:
            ...
            if nova_allocated:
                ...
                if ram:
                    samples.append(
                        self._make_sample(
                            'nova.ram', ram, allocation.project_id, unit='MB'
                        )
                    )
                    ram_total += ram
                ...
                if instances:
                    samples.append(
                        self._make_sample(
                            'nova.instances', instances, allocation.project_id
                        )
                    )
...
class CinderQuotaAllocationPollster(AllocationPollsterBase):
    def get_samples(self, manager, cache, resources):
            ...
            if cinder_allocated:
                for k, v in allocation.get_allocated_cinder_quota().items():
                    if k.startswith('gigabytes_'):
                        ...
                        samples.append(
                            self._make_sample(
                                f'cinder.{zone}', q, allocation.project_id
                            )
                        )
        ...
        for zone, total in cinder_totals.items():
            samples.append(
                sample.Sample(
                    name=f'global.allocations.quota.cinder.{zone}',
                    type=sample.TYPE_GAUGE,
                    unit='GB',
                    volume=total,
                    user_id=None,
    ...
    def get_samples(self, manager, cache, resources):
        ...
        for allocation in resources:
            ...
            if swift_allocated['object']:
                ...
                samples.append(
                    self._make_sample(
                        'swift', swift_allocated, allocation.project_id
                    )
                )
    ...
    def get_samples(self, manager, cache, resources):
        ...
        for allocation in resources:
            ...
            if budget:
                budget = int(budget)
                samples.append(
                    self._make_sample('budget', budget, allocation.project_id)
                )
