...
class ResourceProviderPollster(plugin_base.PollsterBase):
    ...
    def _make_sample(self, metric, value, resource_provider, unit):
        metric_name = f'resource_provider.{metric}'
        LOG.debug(
            "Making sample for %s, rp=%s(%s) value=%s",
            metric_name,
    ...
    def _make_total_sample(self, metric, value, unit):
        return sample.Sample(
            name=f'global.resource_provider.{metric}',
            type=sample.TYPE_GAUGE,
            unit=unit,
            volume=value,
            user_id=None,
    ...
    def _get_capacity(resource):
        return (resource['total'] * resource['allocation_ratio']) - resource[
            'reserved'
        ]
    ...
    def get_samples(self, manager, cache, resources):
        ...
        for resource_provider in resources:
            ...
            if hasattr(usages, 'VCPU'):
                samples.append(
                    self._make_sample(
                        'usage.vcpu', usages.VCPU, resource_provider, 'VCPU'
                    )
                )
            ...
            if hasattr(usages, 'DISK_GB'):
                samples.append(
                    self._make_sample(
                        'usage.disk', usages.DISK_GB, resource_provider, 'GB'
                    )
                )
            ...
            if hasattr(capacity, 'VCPU'):
                ...
                samples.append(
                    self._make_sample(
                        'capacity.vcpu', total, resource_provider, 'VCPU'
                    )
                )
                vcpu_capacity += total
            if hasattr(capacity, 'MEMORY_MB'):
                ...
                samples.append(
                    self._make_sample(
                        'capacity.memory', total, resource_provider, 'MB'
                    )
                )
            ...
            if hasattr(capacity, 'DISK_GB'):
                ...
                samples.append(
                    self._make_sample(
                        'capacity.disk', total, resource_provider, 'GB'
                    )
                )
                disk_capacity += total
        ...
        samples.append(self._make_total_sample('usage.disk', disk_usage, 'GB'))
        samples.append(
