...
class GnocchiSender(base.BaseSender):
    ...
    def send_metric(
        ...
        if by_name:
            terms = [{'=': {'name': resource_name}}, {'=': {'ended_at': None}}]
        else:
            terms = [{'=': {'id': resource_name}}]

        resources = self.client.resource.search(
            resource_type=resource_type, query={'and': terms}
        )
        ...
        if num_resources == 1:
            ...
            resource = self.client.resource.create(
                resource_type=resource_type, resource=args
            )
        else:
            self.log.error(
                f"More than 1 resource exists for type: {resource_type} and "
        ...
        self.log.debug(
            f"Resource: {resource_type}:{resource} metric: {metric}"
        )
        self.client.metric.add_measures(
            metric=metric_id,
            measures=[dict(timestamp=time, value=float(value))],
        )
    ...
    def send_by_host_by_home(self, host, home, metric, value, time):
        ...
        metric = "{}.usage.{}.{}".format(
            _type, home, metric.replace('used_', '').rstrip('s')
        )
    ...
    def send_capacity_by_site(self, site, scope, metric, value, time):
        metric = f"capacity.{scope}.{metric}"
        self.send_metric('site', site, metric, value, time, by_name=True)

    def send_usage_by_site(self, site, scope, metric, value, time):
        metric = f"usage.{scope}.{metric}"
        self.send_metric('site', site, metric, value, time, by_name=True)

    def send_availability_by_site(self, site, scope, metric, value, time):
        metric = f"availability.{scope}.{metric}"
        self.send_metric('site', site, metric, value, time, by_name=True)
